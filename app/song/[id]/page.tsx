  ...
  const fetchSong = async () => {
    ...
    if (error) {
      ...
      parsedSections = parsedSections.map(sec => ({
        title: sec.title || 'חלק',
        bars: Array.isArray(sec.bars) ? sec.bars : [{ chord: 'Am' }]
      }))

      setSong({
        id: data.id,
        title: data.title || 'שיר ללא שם',
        key: data.key || 'Am',
  ...
  const handleLiveTranspose = (direction: number) => {
    ...
    const updatedSections = (song.sections || []).map(sec => ({
      ...
      bars: (sec.bars || []).map(bar => ({
        ...bar,
        chord: transposeChord(bar.chord || 'Am', direction)
      }))
    }))
    ...
    <div style={{ backgroundColor: '#0d1310', minHeight: '100vh', color: '#e8f5e9', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif', direction: 'rtl', padding: '15px', display: 'flex', justifyContent: 'center' }}>
      <div style={{ width: '100%', maxWidth: '450px', backgroundColor: '#111a15', borderRadius: '20px', padding: '20px', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
          <button 
            onClick={() => window.location.href = '/setlist'}
        ...
        {/* אזור פעולות תחתון (לינק השמעה + כפתורי עריכה) */}
        <div style={{ marginTop: '25px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          
          {/* כפתור השמעת השיר המקורי - נראה תמיד אם יש קישור (גם לאדמין וגם לנגן) */}
          {song.audio_url && song.audio_url.trim() !== '' ? (
            <a 
              href={song.audio_url} 
              target="_blank" 
              rel="noopener noreferrer"
              style={{ textDecoration: 'none', display: 'block', width: '100%', padding: '14px', borderRadius: '10px', border: '1px solid #ff9800', background: '#16221c', color: '#ff9800', fontWeight: 'bold', fontSize: '1rem', textAlign: 'center', boxSizing: 'border-box' }}
            >
              🎧 השמעת השיר המקורי
            </a>
          ) : (
            role === 'viewer' && (
              <div style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid #333', background: '#0d1310', color: '#666', fontSize: '0.85rem', textAlign: 'center', boxSizing: 'border-box' }}>
                🎧 לא הוגדר קישור לשיר מקורי
              </div>
            )
          )}

          {/* כפתורי עריכת מבנה ואקורדים - מופיעים מתחת ללינק (לאדמין בלבד) */}
          {role === 'admin' && (
